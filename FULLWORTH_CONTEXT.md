...
- Development is `af3501202d8d7f2946bf7469c5887ae4b8b4a24b` after PR #217 merged the legacy-compatibility guard. PR #217 exact head `b5380e6877016a8504b40e9fd8e37fb2459c344a` passed FullWorth CI #756 across backend build/tests, MAUI Android, and Linux production/security/recovery gates before merge.
- PR #216 merged the private AI corpus path migration as `3ce76e60b26e0b0a062e9d5ed97014ad1a2068ec` after exact-head CI #750. The canonical temporary local path is now `.private/FullWorth.AiShadowCorpus/`; legacy BillWatch corpus paths remain ignored so an existing sensitive local corpus cannot become accidentally trackable.
- PR #215 merged the safe readiness/test-only branding batch as `2a5952631d944ca22860e36b9c9fcd0c7ad21c1f` after exact-head CI #745. PR #214 merged the operator-visible shell/systemd/smoke branding batch as `706dabed36a989009a1fcfe11dd2709eb6c9dac6` after exact-head CI #735. PRs #212/#213 handled the corresponding operator/private-beta documentation cleanup.
- The BillWatch → FullWorth cosmetic/operator migration is now complete up to the explicit compatibility boundary. CI now fails closed if a future change accidentally renames frozen secure-storage, claim/cookie/Data Protection, browser preference, Stripe metadata, external-auth, telemetry, alert/backup, legacy sender/domain, proof-confirmation, deployment-path, private-corpus, or context-pointer identifiers without an explicit migration.
- PR #210 remains open from `development` to `master`, currently tracking development head `af3501202d8d7f2946bf7469c5887ae4b8b4a24b`. Historical CI #726 covered an older promotion head only and is not release evidence for the current head.
...
- The remaining release/UI gate is real installed-device / interaction acceptance: installed Android/iOS PWA behavior where available, keyboard/back behavior, installed-PWA update flow, Plaid Hosted Link return, statement file picker, and installed/mobile security-dialog behavior. Do not promote PR #210 merely because repository CI is green; any release promotion still needs exact-head CI plus the required installed-device evidence documented below.
