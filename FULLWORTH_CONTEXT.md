...
- PR #210 remains open from `development` to `master` and tracks the moving `development` head. Do not promote #210 until the required installed-device interaction evidence exists for the exact release candidate and that promotion head has exact-head CI. Any CI tied to an earlier development head is stale release evidence.
