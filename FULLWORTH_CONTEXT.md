...
- Development is `10834fd358c8f44e09a479897637f34283976020` after PR #213 merged the broader operator/private-beta documentation branding batch. Its exact head `4efab24db3547236b5d7b6506d5dd80228ce9085` passed FullWorth CI #730 across backend build/tests, MAUI Android, and Linux production-container gates before merge.
- PR #212 previously merged the first operator-guide FullWorth branding cleanup after exact-head CI #727; PR #208/#209/#211 established canonical FullWorth legal, identity-email, Stripe return-URL, monitoring, and operations-documentation defaults.
- PR #210 remains open from `development` to `master`. CI #726 passed only for its earlier head `163c1b9f59cf4e1b26dc855999029b84abf22916`; later development merges mean that result is not exact-head release evidence for the current promotion head.
- The current migration batch moves operator-visible shell/systemd/smoke output from BillWatch to FullWorth while deliberately preserving compatibility-sensitive `BILLWATCH_*` variables, `/opt/billwatch`, `.billwatch-release`, systemd unit filenames, source identifiers, persisted backup tags, confirmation strings consumed by proof scripts, database/configuration identifiers, and other runtime/external identities. Matching regression expectations change only where they assert the renamed operator-visible text.
