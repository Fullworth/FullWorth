...
- Development advanced to `939cef6912b171b28acf30ec43383e7a467f0cef` after PR #212 merged the first operator-guide FullWorth branding cleanup. PR #212 exact head `6cefe14a1ddaf65cf53198b902ab902fcbc36472` passed FullWorth CI #727 across backend build/tests, MAUI Android, and Linux production-container gates before merge.
- PR #208 previously merged canonical FullWorth legal, identity-email, and Stripe return-URL defaults; PR #209 merged canonical API/Web monitoring targets; PR #211 merged the matching operations-documentation handoff as `163c1b9f59cf4e1b26dc855999029b84abf22916`.
- PR #210 remains open from `development` to `master`. CI #726 passed when its head was `163c1b9f59cf4e1b26dc855999029b84abf22916`, but later development documentation merges move that promotion head forward, so #726 must not be treated as exact-head release evidence for a newer promotion head.
- This migration batch expands the safe presentational rename through operator/private-beta documentation and replaces remaining documentation-only `api.billwatch.example` samples with the canonical `https://api.fullworth.org` endpoint. It deliberately preserves compatibility-sensitive `BILLWATCH_*` variables, `/opt/billwatch`, `.billwatch-release`, state paths, confirmation strings consumed by scripts, service names, database/configuration identifiers, and other runtime/external identities.
...
- After this documentation batch, continue with the outstanding installed-device / real-environment acceptance evidence. Do not promote #210 merely because documentation CI is green; any eventual release promotion still needs exact-head CI and the required device/interaction evidence documented below.
