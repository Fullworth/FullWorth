...
- Development is currently `b7ae2d975ece59959f4c239bdb93d33bfd9dc43c` after PR #218 merged the refreshed migration checkpoint. PR #218 exact head `09622d2a8d8bf1c2779766625954548d2a22def6` passed FullWorth CI #763 before merge.
- PR #217 merged the legacy-compatibility guard as `af3501202d8d7f2946bf7469c5887ae4b8b4a24b` after exact-head CI #756. PR #216 merged the private AI corpus path migration as `3ce76e60b26e0b0a062e9d5ed97014ad1a2068ec` after CI #750; PR #215 and PR #214 completed the safe test/operator-visible branding layers after CI #745 and #735 respectively.
- The BillWatch → FullWorth cosmetic/operator migration is complete up to the explicit compatibility boundary. CI fails closed if a future change accidentally renames frozen secure-storage, claim/cookie/Data Protection, browser preference, Stripe metadata, external-auth, telemetry, alert/backup, legacy sender/domain, proof-confirmation, deployment-path, private-corpus, or context-pointer identifiers without an explicit migration.
- PR #219 is the next active checkpoint. Current exact head `fe81a2274c9e2ea7b1ca185e18a0ae7ea3958087` adds release-pinned human installed-device acceptance evidence. Android evidence becomes required by the same-release private-beta acceptance bundle; iOS evidence is verified and included when supplied. The recorder stores metadata only and does not record device identifiers, account data, screenshots, statement contents, credentials, tokens, or provider responses. PR #219 must pass exact-head CI before merge.
- The installed-device evidence phase set covers installed PWA launch, keyboard resize, Back/navigation behavior, controlled PWA update, statement file picker, and installed/mobile security dialogs. Plaid Hosted Link return remains independently proven by the existing Plaid observation evidence. The evidence harness records human attestation; it does not replace performing the real device checks.
- PR #210 remains open from `development` to `master`, currently tracking development head `b7ae2d975ece59959f4c239bdb93d33bfd9dc43c`. Any CI tied to an earlier promotion head becomes stale when development advances. Do not promote #210 until the required installed-device interaction evidence exists for the exact release candidate and the promotion head has exact-head CI.
