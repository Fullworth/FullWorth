...
[ -d "$root_dir/deploy/systemd" ] ||
    fail "FullWorth systemd source directory was not found under $root_dir." 64
...
do
    ...
    [ -f "$source_path" ] || fail "Required FullWorth systemd unit is missing: $unit" 64
    [ ! -L "$source_path" ] || fail "FullWorth systemd unit must not be a symbolic link: $unit" 64
...
printf '%s\n' "FullWorth production systemd units installed with mode 0644."
