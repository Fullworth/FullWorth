...
payload="{\"text\":\"FullWorth readiness alert: $event_safe for $target_safe (run $run_safe).\",\"source\":\"billwatch-external-readiness\",\"event\":\"$event_safe\",\"target\":\"$target_safe\",\"runId\":\"$run_safe\",\"occurredAtUtc\":\"$timestamp\"}"
