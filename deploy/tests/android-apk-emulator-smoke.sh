...
ANDROID_USER_HOME="${RUNNER_TEMP:-/tmp}/fullworth-android-user"
...
printf '%s\n' no |
    avdmanager create avd \
        ...
        --device pixel_6 \
        --path "$ANDROID_AVD_HOME/$avd_name.avd" >/dev/null

avdmanager list avd |
    grep -F "Name: $avd_name" >/dev/null ||
    fail "Android AVD was created but is not visible to avdmanager." 70

[ -f "$ANDROID_AVD_HOME/$avd_name.ini" ] ||
    fail "Android AVD metadata was not created in the isolated AVD home." 70
...
while [ "$attempt" -lt 60 ]
do
    if adb get-state 2>/dev/null | grep -Fx 'device' >/dev/null 2>&1; then
        device_ready=true
        break
    fi

    if ! kill -0 "$emulator_pid" 2>/dev/null; then
        fail "Android emulator exited before ADB became ready." 70
    fi
...
[ "$device_ready" = true ] ||
    fail "Android emulator did not become reachable through ADB within 120 seconds." 70
...
[ "$booted" = true ] || fail "Android emulator did not finish booting within 180 seconds." 70
